...
class CartServiceTest {
    ...
    @Test
    @DisplayName("요약/통계: 총/유형별/최대/평균 금리 계산")
    void getCartSummary_ok() {
        Long userId = 1L;

        given(cartRepository.countByUserId(userId)).willReturn(5L);
        ...
        given(cartRepository.findMaxInterestRateByUserId(userId))
                .willReturn(Optional.of(BigDecimal.valueOf(4.5)));

        Cart top = mock(Cart.class);
        given(top.getProductName()).willReturn("최고금리 상품");
        given(top.getMaxInterestRate()).willReturn(BigDecimal.valueOf(4.5));

        Cart c2 = mock(Cart.class);
        given(c2.getMaxInterestRate()).willReturn(BigDecimal.valueOf(3.5));

        given(cartRepository.findByUserIdOrderByMaxInterestRateDesc(userId))
                .willReturn(List.of(top, c2));

        CartSummaryDTO sum = cartService.getCartSummary(userId);

        assertThat(sum.getTotalCount()).isEqualTo(5);
        ...
        assertThat(sum.getMaxInterestProductName()).isEqualTo("최고금리 상품");
        // 평균 (4.5 + 3.5) / 2 = 4.00
        assertThat(sum.getAvgInterestRate()).isEqualByComparingTo("4.00");
    }
