...
public class SavingFilterServiceTest {
  ...
  @Test
  @DisplayName("적립방식 필터링")
  void testReserveTypeFiltering() {
    // Given
    SavingFilterRequest request = SavingFilterRequest.builder()
        .filters(SavingFilterRequest.Filters.builder()
            .rsrvType(Arrays.asList("F")) // 자유적립식
            .build())
        .build();
    request.setPage(1);
    request.setSize(10);
  ...
  @Test
  @DisplayName("월 저축금 한도 필터링")
  void testMonthlyMaxLimitFiltering() {
    // Given
    SavingFilterRequest request = SavingFilterRequest.builder()
        .filters(SavingFilterRequest.Filters.builder()
            .monthlyMaxLimit(600000) // 월 60만원 저축 원함
            .build())
        .build();
    request.setPage(1);
    request.setSize(10);
  ...
  @Test
  @DisplayName("총 저축금 계산 필터링 - 기간 지정")
  void testTotalMaxLimitFilteringWithPeriod() {
    // Given
    SavingFilterRequest request = SavingFilterRequest.builder()
        .filters(SavingFilterRequest.Filters.builder()
            .saveTrm(Arrays.asList(24)) // 24개월
            .totalMaxLimit(15000000) // 총 1,500만원 저축 원함
            .build())
        .build();
    request.setPage(1);
    request.setSize(10);
  ...
  @Test
  @DisplayName("총 저축금 계산 필터링 - 기간 미지정")
  void testTotalMaxLimitFilteringWithoutPeriod() {
    // Given
    SavingFilterRequest request = SavingFilterRequest.builder()
        .filters(SavingFilterRequest.Filters.builder()
            .totalMaxLimit(10000000) // 총 1,000만원 저축 원함
            .build())
        .build();
    request.setPage(1);
    request.setSize(10);
    ...
    // Then
    assertThat(result).isNotNull();
    // 각 상품의 최대 기간으로 계산해서 1,000만원 이상 저축 가능한 상품들
    assertThat(result.getContent()).hasSize(2); // 고금리, 중간금리 상품만
  }
  ...
  void testPagination() {
    ...
    // 두 번째 페이지 요청
    SavingFilterRequest secondPageRequest = SavingFilterRequest.builder()
        .filters(null)
        .build();
    secondPageRequest.setPage(2);
    secondPageRequest.setSize(2);

    Page<ProductSummaryResponse> secondPage = savingFilterService.savingFilter(secondPageRequest);
