...
public class SensSmsGateway {
  ...
  public void send(String to, String content) {
    ...
    final String sig = makeSignature("POST", path, timestamp, smsProperties.getAccessKey(),
    ...
    webClient.post()
        ...
        .onStatus(s -> s.value() == 408 || s.value() == 429 || s.is5xxServerError(),
            r -> r.bodyToMono(String.class)
                .map(b -> new RetryableChannelException(
                    "SENS retryable: " + r.statusCode() + " " + b)))
        // 그 외 4xx -> 영구 실패
        .onStatus(HttpStatusCode::is4xxClientError,
            r -> r.bodyToMono(String.class)
                .map(b -> new NonRetryableChannelException(
                    "SENS non-retryable: " + r.statusCode() + " " + b)))
        .toBodilessEntity()
        .onErrorMap(t ->
            (t instanceof RetryableChannelException || t instanceof NonRetryableChannelException) ?
                t : new RetryableChannelException("SENS transport error", t))
        .block();
  }

  // SENS 시그니처 생성
  private String makeSignature(String method, String path, String timestamp, String accessKey,
      String secretKey) {
    try {
