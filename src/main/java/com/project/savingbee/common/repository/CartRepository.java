...
/**
 * 장바구니 데이터 접근을 위한 Repository 사용자의 관심 상품 관리 및 비교 분석에 활용
 */
@Repository
public interface CartRepository extends JpaRepository<Cart, Long> {
  ...
  // 상품 타입별 필터링 + 페이징 (최근순)
  Page<Cart> findByUserIdAndProductTypeOrderByCreatedAtDesc(Long userId,
      Cart.ProductType productType, Pageable pageable);

  // 상품 타입별 필터링 + 페이징 (금리순)
  @Query("SELECT c FROM Cart c WHERE c.userId = :userId AND c.productType = :productType ORDER BY c.maxInterestRate DESC")
  Page<Cart> findByUserIdAndProductTypeOrderByMaxInterestRateDesc(@Param("userId") Long userId,
      @Param("productType") Cart.ProductType productType, Pageable pageable);
  ...
  // 금리순 정렬 + 페이징
  @Query("SELECT c FROM Cart c WHERE c.userId = :userId ORDER BY c.maxInterestRate DESC")
  Page<Cart> findByUserIdOrderByMaxInterestRateDesc(@Param("userId") Long userId,
      Pageable pageable);
