...
/**
 * 사용자 보유 예적금 상품 데이터 접근을 위한 Repository 추천 시스템의 기준 데이터 조회 및 관리
 */
@Repository
public interface UserProductRepository extends JpaRepository<UserProduct, Long> {
  ...
  // 특정 상품 타입별 조회
  List<UserProduct> findByUserIdAndProductTypeAndIsActiveTrue(Long userId,
      UserProduct.ProductType productType);

  // 만기일 기준 조회 (알림용)
  @Query("SELECT up FROM UserProduct up WHERE up.isActive = true AND up.maturityDate BETWEEN :startDate AND :endDate")
  List<UserProduct> findByMaturityDateBetween(@Param("startDate") LocalDate startDate,
  ...
  List<UserProduct> findByUserIdAndMaturityDateAndIsActiveTrue(@Param("userId") Long userId,
  ...
  // 은행별 보유 상품 조회
  List<UserProduct> findByUserIdAndBankNameContainingIgnoreCaseAndIsActiveTrue(Long userId,
      String bankName);
}