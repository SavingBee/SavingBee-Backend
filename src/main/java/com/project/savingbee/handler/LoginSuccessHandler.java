...
public class LoginSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
    ...
    System.out.println(
        "Access token created: " + accessToken.substring(0, Math.min(20, accessToken.length()))
            + "...");
    System.out.println(
        "Refresh token created: " + refreshToken.substring(0, Math.min(20, refreshToken.length()))
            + "...");
    ...
    String json = String.format("{\"accessToken\":\"%s\", \"refreshToken\":\"%s\"}", accessToken,
        refreshToken);
