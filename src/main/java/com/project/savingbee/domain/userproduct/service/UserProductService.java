...
/**
 * 사용자 보유 예적금 상품 관리 서비스 실제 보유 상품 CRUD 및 추천 시스템의 기준 데이터 관리
 */
@Service
...
public class UserProductService {
  ...
  /**
   * 사용자 보유 상품 등록 (기존 메서드) 예) "KB국민은행 / 정기예금 / 가입 기간 12개월 / 금리 3.2% / 가입일 2025-10-01 / 만기일
   * 2026-10-01"
   */
  @Transactional
  ...
  public UserProductPageResponseDTO getUserProducts(String username, Pageable pageable) {
    ...
    Page<UserProduct> productPage = userProductRepository.findByUserIdAndIsActiveTrue(userId,
        pageable);

    List<UserProductResponseDTO> content = UserProductResponseDTO.fromList(
   ...
   */
  public List<UserProductResponseDTO> getUserProductsList(Long userId,
      UserProductRequestDTO request) {
    ...
    } else if (request.getFilterBankName() != null && !request.getFilterBankName().trim()
        .isEmpty()) {
   ...
   */
  @Transactional
  public UserProductResponseDTO updateUserProductById(Long userId, Long userProductId,
      UserProductRequestDTO request) {
    UserProduct userProduct = userProductRepository.findById(userProductId)
        .orElseThrow(() -> new IllegalArgumentException("보유 상품을 찾을 수 없습니다."));
  ...
  public List<UserProductResponseDTO> getMaturityProducts(int daysBefore, Long userId) {
    ...
    List<UserProduct> products = userProductRepository.findByUserIdAndMaturityDateAndIsActiveTrue(
        userId, targetDate);
    return UserProductResponseDTO.fromList(products);
  }
  ...
  public UserProductSummaryDTO getUserProductSummary(Long userId) {
    ...
    return UserProductSummaryDTO.builder()
        ...
        .averageInterestRate(activeProducts.stream()
            ...
            .reduce(java.math.BigDecimal.ZERO, java.math.BigDecimal::add)
            .divide(new java.math.BigDecimal(Math.max(1, activeProducts.size())), 2,
